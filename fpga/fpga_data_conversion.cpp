...
{
    auto it = this->CHANNEL_NAME__TO__CHANNEL_INDEX.find(channelName);
    PARAM_CHECK(it != this->CHANNEL_NAME__TO__CHANNEL_INDEX.end(), "fpga", " in [SignalData::GetChannelData] Invalid channel name: " + channelName);
    data->assign(this->_channelData[it->second].begin(), this->_channelData[it->second].end());
}

void vuprs::SignalData::_UpdataHashMap()
{
    PARAM_CHECK(this->_channelData.size() == this->_channelName.size(), "fpga", " in [SignalData::_UpdataHashMap] Unmatch channel & name size.");
...
{
    this->_UpdataHashMap();
    PARAM_CHECK(!(this->_channelData.empty() || this->_channelName.empty()), "fpga", " in [SignalData::ToCSV] No channel data to export.");
    std::string dir;
    ...
    RUNTIME_CHECK(file.is_open(), "fpga", " in [SignalData::ToCSV] Cannot open output file: " + outputFile);
        ...
        if ((headerPointer + 1) < vectorSize && (tailerPointer + 1) < vectorSize)
        {
            ...
            for (int ch = 0; ch < ADC_CHANNEL_NUMBER; ch++)
            {
                adcData->_channelData[ch].emplace_back(
                    std::complex<double>(
                        vuprs::Q15__ADC_UINT16_TO_DOUBLE(rawBufferDataVector[headerPointer + ch + 2],
                                                         v_scale),
                        0.0));
            }
            frameParsedSize++;
