...
bool vuprs::FIRCalculator::ConfigFIRFromJsonFile(const std::string &jsonFilename)
{
    ...
    RUNTIME_CHECK(arrayConfigJsonFile.is_open(), "bf", " in [FIRCalculator::ConfigFIRFromJsonFile] Cannot open file: " + jsonFilename);
    ...
    {
        RUNTIME_CHECK(false, "bf", " in [FIRCalculator::ConfigFIRFromJsonFile] Failed to load array data from: " + jsonFilename);
    }
...
{
    PARAM_CHECK(this->configdone, "bf", " in [FIRCalculator::SolveCoeffUseExpectedFrequencyResponse] Config not complete.");
    int M = response.rows();          /* M */
    int N_2_plus_1 = response.cols(); /* N/2+1 */
    int N = (N_2_plus_1 - 1) * 2;     /* N */
    PARAM_CHECK(N >= this->firLength, "bf", " in [FIRCalculator::SolveCoeffUseExpectedFrequencyResponse] Too little response for solving.");
    ...
    futures.reserve(M);
    if (this->fftManagers.size() != M)
    {
        this->fftManagers.resize(M);
    }
    for (auto &manager : this->fftManagers)
    {
    ...
    for (int i = 0; i < M; i++)
    {
        futures.emplace_back(this->threadPool->enqueue(
            [&, i]()
            {
                ...
                /* IFFT */
                this->fftManagers[i].DoDFT(Hd.data(), h.data());
                h /= (double)N;
                /* Get Real */
...
{
    PARAM_CHECK(this->configdone, "bf", " in [FIRCalculator::GetZeroFIRBankCoefficient] Config not complete.");
    PARAM_CHECK(this->firLength > 0, "bf", " in [FIRCalculator::GetZeroFIRBankCoefficient] FIR length = 0");
