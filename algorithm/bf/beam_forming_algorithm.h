...
namespace vuprs
{
    struct IterationConfig
    {
        ...
        double lower_region;                /* lower boundary */
        double upper_region;                /* upper boundary */
        double jumpout;                     /* jumpout threshold */
        int max_iteration;                  /* max iteration counts */
    };
    ...
    public:
        ...
        std::future<typename std::result_of<F(Args...)>::type> enqueue(F &&f, Args &&...args)
        {
            ...
            std::future<return_type> result = task->get_future();
            {
                ...
                RUNTIME_CHECK(!this->stop, "bf", " Enqueue on stopped ThreadPool");
                tasks.emplace([task]()
                              { (*task)(); });
            }
     ...
     */
    inline Eigen::Matrix<double, 3, 1> AltAz2PointingVector(double alt,
                                                            double az)
    {
     ...
     */
    inline void PointingVector2AltAz(const Eigen::Matrix<double, 3, 1> &vec,
                                     double *alt,
                                     double *az)
    {
     ...
     *
     * @note result = Solve(func(val) == 0), val in [lower_region, upper_region].
     *
     ...
     * @param exp_matrix output matrix E.
     * @param use_positive_freq true: E.size = (N/2+1) x L, false: E.size = N x L.
     */
    void Get_FIR_EXPMatrix(int L_fir,
                           int N_points,
                           Eigen::Matrix<Eigen::dcomplex, -1, -1> *exp_matrix,
                           bool use_positive_freq);
